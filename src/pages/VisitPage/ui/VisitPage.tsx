...
export const VisitPage = () => {
  ...
  const groupCourses = groups.filter(g => g.course === value && g.institute_id === selectedItems);
  const groupStudents = students.filter(st => st.group_id === stud);
  const groupDisciplines = disciplines.filter(discipline => discipline.group_id === group);
    ...
    <div className={styles.visitPage}>
      ...
      >
        <Space direction="horizontal">
          {
            courses.map(item => <Radio value={item.id}>{item.name}</Radio>)
          }
        </Space>
      </Radio.Group>
      <Flex gap="small" wrap style={{ marginTop: '20px' }}>
        {
          groupCourses.map(gr => <Button onClick={() => groupId(gr.id)}>{gr.name}</Button>)
        }
      </Flex>
      <Flex gap="small" style={{ width: '50%', marginTop: '20px', display: 'flex', flexDirection: 'column' }}>
        {
          groupDisciplines.map(disc => <Button onClick={() => studId(disc.group_id)} type="primary">{disc.name}</Button>)
        }
      </Flex>
      { stud &&
      <table>
        <tr>
          ...
          {
            selectedGroup?.date.map(d => <th>{d}</th>)
          }
        </tr>
        {
          groupStudents.map((stud, i) => <tr>
            <td className={styles.num}>{i + 1}</td>
            <td className={styles.fio}>{stud.name}</td>
            {
              stud.visit.map(vis => <td style={vis === '+'? {color: 'green'}:{color: 'red'}}>{vis}</td>)
            }
          </tr>)
        }
      </table> }
