...
import Departments from "stores/DB/Departments.json";
...
export const TeacherPage = () => {
  const { id } = useParams();
  const teacher = Teachers.teachers.find((item) => String(item.id) === id);
  const department = Departments.departments.find(
    (item) => item.id === teacher.department_id
  );
  const institute = Institutes.institutes.find(
    (item) => item.id === department.institute_id
  );
  ...
  return (
    <div className={styles.teacherPage}>
      <div className={styles.teacherInfo}>
        ...
        <Text>Кафедра: {department.name}</Text>
